...
def calc_apply(operator, args):
    ...
    if operator in ('round'):
        ...
        if type(args[0]) is int:
            raise TypeError('You can round only floats')
...
known_operators = ['add', 'sub', 'mul', 'div', '+', '-', '*', '/'] + ['eq','lt','gt','==','<','>'] + ['round']
...
def analyze(tokens):
    ...
    if type(token) in (int, float):
        return token
    if token not in known_operators and token in vars:
        return vars[token]
    if token in known_operators:
        if len(tokens) == 0 or tokens.pop(0) != '(':
            raise SyntaxError('expected ( after ' + token)
        return Exp(token, analyze_operands(tokens))
    elif tokens.pop(0) in ('='):
        vars[token] = calc_eval(analyze(tokens))
    else:
